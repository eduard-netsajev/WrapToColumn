...
class LineWrapHandler(val useCaretColumn: Boolean) : EditorActionHandler() {
    ...
    override fun execute(editor: Editor, dataContext: DataContext?) {
        ApplicationManager.getApplication().runWriteAction(object : Runnable {
            override fun run() {
                ...
                val wrapper = getWrapper(project, editor, fileIsPlaintext, useCaretColumn)
                val text: String
                var start: Int
                val end: Int
                ...
                if (selectionModel.hasSelection()) {
                    ...
                    val possibleWhitespaceEnd = if (start == 0) start else Math.max(start - 1, lineStartOffset)
